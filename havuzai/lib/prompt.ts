...
export function buildPoolPrompt(config: PoolConfig, clientConfig: ClientConfig): string { 
  ...
  // Seçilen modelin açıklaması havuzun kesin geometrisini tanımlar
  const shapeRule = 
...
First create the EXACT pool shape from Image 2 and the selected model description. 
Keep the pool shape unchanged. 
Then add the ceramic tiles OUTSIDE the completed pool. 
The ceramic tiles must follow the pool outline — the pool shape must NEVER change to fit the tiles. 

Add a ceramic tile walkway around the ENTIRE OUTER PERIMETER of the pool. 
...
- Tiles laid following the pool edge, with the LONG side (66cm) parallel to the nearest pool edge 
...
First create the EXACT pool shape from Image 2 and the selected model description. 
Keep the pool shape unchanged. 
Then add the deck OUTSIDE the completed pool. 
The deck must follow the pool outline — the pool shape must NEVER change to fit the deck. 

Add a composite wood deck around the ENTIRE OUTER PERIMETER of the pool. 
- DO NOT add any white border, coping, or rim around the pool. 
- Exactly 3 deck boards around the pool — total width 60cm 
- Each board is 20cm wide, laid following the nearest pool edge 
